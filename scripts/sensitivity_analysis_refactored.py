...
class RefactoredSensitivityAnalyzer:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            file_path_params = (
                project_root / "data/lookup tables/mobility-model-parameters.csv"
            )
            file_path_sens_vars = (
                project_root / "data/lookup tables/Sensitivity Analysis Variables.csv"
            )
            ...
            param_df = all_hpv_param_df.loc[
                all_hpv_param_df["Name"] == self.filter_value
            ]
            ...
            logger.info(
                f"Loaded {len(param_df)} parameter records and {len(sens_df)} sensitivity variables"
            )
    ...
    def initialize_model_components(self, param_df: pd.DataFrame) -> Tuple:
        ...
        try:
            ...
            met = mm.MET_values(
                mv, country_weight=60, met=4.5, use_country_specific_weights=False
            )
    ...
    def calculate_metrics(self, velocities: List[Dict], mv) -> Dict[str, float]:
        ...
        # Time per round trip
        time_per_trip_hours = (
            round_trip_distance_km / (avg_velocity * 3.6)
            if avg_velocity > 0
            else np.inf
        )
        ...
        max_trips_in_time = (
            mv.t_hours / time_per_trip_hours if time_per_trip_hours > 0 else 0
        )
        daily_water_collected = min(
            max_trips_in_time * self.fixed_water_capacity,
            ...
            try:
                if mo.model_selection == 2:  # Cycling (Martin model)
                    ...
                    if isinstance(result, tuple) and len(result) == 3:
                        ...
                        logger.error(
                            f"Unexpected result format from single_bike_run: {type(result)}, {result}"
                        )
                        continue
                    ...
                    if isinstance(result, tuple) and len(result) == 3:
                        ...
                        logger.error(
                            f"Unexpected result format from single_lankford_run: {type(result)}, {result}"
                        )
                        continue
                else:
                ...
                loaded_velocity = (
                    float(loaded_velocity) if not np.isnan(loaded_velocity) else np.nan
                )
                unloaded_velocity = (
                    float(unloaded_velocity)
    ...
    def apply_sensitivity_parameter(
        self, var_string: str, var_test: float, hpv, mv, mo, met
    ) -> Tuple:
        """
        ...
        if var_string == "Coefficient of Rolling Resistance":
            ...
            met = mm.MET_values(
                mv, country_weight=60, met=var_test, use_country_specific_weights=False
            )
    ...
    def run_single_sensitivity(
        self, sens_row: pd.Series, param_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        for var_test in phase_space:
            try:
                ...
                logger.error(
                    f"Error in sensitivity calculation for {var_string} = {var_test}: {e}"
                )
        ...
        df_results["Adjusted Distance"] = (
            df_results["One-way Distance (km)"] - default_distance
        )
    ...
    def create_phase_space(
        self, sens_row: pd.Series, resolution: int = 7
    ) -> np.ndarray:
        ...
        phase_space = np.linspace(
            start=plot_min, stop=plot_max, num=resolution, endpoint=True
        )
    ...
    def run_full_sensitivity_analysis(self) -> pd.DataFrame:
        ...
        for i, (_, sens_row) in enumerate(sens_df.iterrows()):
            ...
            try:
                ...
                logger.info(
                    f"Completed sensitivity analysis for {var_string} ({i+1}/{len(sens_df)})"
                )
    ...
    def create_summary_plot(
        self, mo, show_default_annotations: bool = False
    ) -> go.Figure:
        """
        ...
        pivot_df = summary_df.pivot(
            index="Name", columns="DataType", values="Adjusted Distance"
        )
        pivot_df["abs_diff"] = np.abs(
            pivot_df["Maximum Expected"] - pivot_df["Minimum Expected"]
        )
    ...
    def save_results(self, output_dir: Optional[str] = None) -> None:
        ...
        try:
            ...
            summary_file = (
                output_path
                / f"sensitivity_analysis_{self.model_type.lower()}_refactored.csv"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Refactored Water Access Sensitivity Analysis"
    )
    parser.add_argument(
        "--model",
        "-m",
        type=str,
        choices=["Lankford", "Martin"],
        default="Lankford",
    ...
    try:
        ...
        summary_fig = analyzer.create_summary_plot(
            mo, show_default_annotations=args.show_defaults
        )
        ...
        if not default_results.empty:
            ...
            print(
                f"  Round-trip distance: {default_row['Round-trip Distance (km)']:.2f} km"
            )
            print(
                f"  Average velocity: {default_row['Average Velocity (m/s)']:.2f} m/s"
            )

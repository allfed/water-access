...
missing_data_counts = df[['URBANPiped', 'RURALPiped', '% urban', 'TOTALPiped']].isna().sum()

print("Missing Data Counts:")
print(missing_data_counts)

# Check correlation of missing data
missing_correlation = df[['URBANPiped', 'RURALPiped', '% urban', 'TOTALPiped']].isna().corr()
...
urban_present_rural_missing = df[
    df['URBANPiped'].notna() &
    df['RURALPiped'].isna()
]

# Filter where RURALPiped is present but URBANPiped is missing
rural_present_urban_missing = df[
    df['RURALPiped'].notna() &
    df['URBANPiped'].isna()
]
...
urban_or_rural_present_total_missing = df[
    (df['URBANPiped'].notna() | df['RURALPiped'].notna()) &
    df['TOTALPiped'].isna()
]
...
interesting_columns = ['Country', 'Year', 'URBANPiped', 'RURALPiped', '% urban', 'TOTALPiped']

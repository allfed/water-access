...
columns_to_check = ['dtw_nan', 'pop_density_nan', 'GHS_SMOD_nan', 'slope1_nan', 'any_nan']
...
print(f"Number of population less than 0.1 in World: {len(df_no_antarctica[df_no_antarctica['pop_density'] < 0.1])}")

# NaN
print(f"Number of NaN in World: {len(df_no_antarctica[df_no_antarctica['pop_density'].isna()])}")

# Total
print(f"Total number of World: {len(df_no_antarctica)}")

# percentage of the total
print(f"Percentage of population less than 0.1 in World: {len(df_no_antarctica[df_no_antarctica['pop_density'] < 0.1]) / len(df_no_antarctica) * 100:.2f}%")
print(f"Percentage of NaN in World: {len(df_no_antarctica[df_no_antarctica['pop_density'].isna()]) / len(df_no_antarctica) * 100:.2f}%")

...
def calculate_average_weight_per_country(bmi_women_file, bmi_men_file, height_file):
    ...
    average_weight_df.rename(columns={'Code': 'alpha3', 'Entity': 'Country', 'Overall Average Weight (kg)': 'Average Weight'}, inplace=True)
...
def main(water_JMP_file_path, bicycle_file_path, gdp_per_capita_file_path, bmi_women_file ,bmi_men_file ,height_file, population_file_path, country_info_output_file_path, household_size_file_path, country_regions_file_path):
    """
    ...
    df_cleaned_merge = df_cleaned_merge.merge(country_regions_df, on='alpha3', how='left')
    ...
    list_of_vars = ['RURALPiped', 'URBANPiped', 'PBO', 'Average Weight', 'Household_Size']
    ...
    extra_vars = ['alpha2', 'Continent', '% urban', 'Population', 'Entity','region', 'subregion', 'intermediate-region']

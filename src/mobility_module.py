...
class mobility_models:
    ...
    def single_lankford_run(mv, mo, met, hpv, slope, load_attempted):
        ...
        # Calculate unloaded velocity. Apply the unloaded hill polarity to the
        # slope (mirroring single_bike_run); the polarity-signed slope flows
        # through to Lankford_solution and is clipped to the validated range
        # there.
        ...
        # cannot be made, so reachable velocity is 0 (never NaN/negative).
        if V_un[2] == 1 and V_un[0][0] >= 0:
            unloaded_velocity = V_un[0][0]
        else:
            unloaded_velocity = 0.0

        # Calculate loaded velocity. Apply the loaded hill polarity to the slope.
    ...
    def Lankford_solution(p, *data):
        ...
        return (
            ...
            - (
                0.000320 * v_solve * G**3
            )  # negative sign per Lankford 2020 fitted model
